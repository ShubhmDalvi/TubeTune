...
(function() {
  ...
  let configReceived = false; // true once the real saved config has arrived from content.js
  ...
  function tryApplyQuality() {
    if (!configReceived) {
      // Don't touch quality until we know the user's real saved preference —
      // otherwise a fresh tab can apply the hardcoded 1080p default before
      // the actual config (e.g. 720p) has had a chance to arrive.
      log('Waiting for saved config before setting quality');
      return null;
    }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.source === "yt-quality-extension" && event.data.action === "init") {
      ...
      const wasFirstConfig = !configReceived;
      ...
      if (wasFirstConfig) {
        // This is the first real config we've received — kick things off now
        // that we actually know the user's saved quality preference.
        log('TubeTune received saved config, starting');
        handleNavigation();
        return;
      }
  ...
  // Tell content.js we're loaded and listening, so it can safely hand us the
  // saved settings without racing our own script injection/parse time.
  window.postMessage({ source: 'yt-quality-page-script', action: 'ready' }, '*');

  // Safety net: if content.js's config never arrives for some reason (e.g.
  // extension context got invalidated), don't sit idle forever — fall back
  // to local defaults after a short grace period so the extension still works.
  setTimeout(() => {
    if (!configReceived) {
      log('No config received in time, falling back to local defaults');
      configReceived = true;
      handleNavigation();
    }
  }, 2000);

  log('TubeTune page script loaded, waiting for saved config...');
